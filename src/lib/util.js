...
export const valueOf = (p, key, brands) => key === "brand" ? brands.find((b) => b.id === p.brandId)?.name || "" : key === "margin" ? marginOf(p) : key === "name" ? p.name || p.contents : (key === "createdAt" || key === "updatedAt") ? fmtDateTime(p[key]) : p[key] ?? "";
