...
import { listUsers, addUser, removeUser, usingSupabase, auth } from "../lib/storage.js";
...
export default function UsersPanel({ me, say }) {
  const [users, setUsers] = useState([]); const [email, setEmail] = useState(""); const [role, setRole] = useState("editor"); const [pw, setPw] = useState("");
  ...
  return (<>
    <div className="panel">
      <h3 style={{ fontSize: 16, marginBottom: 6 }}>My login</h3>
      <div className="note" style={{ marginBottom: 8 }}>Signed in as <b>{me}</b>. Set or change your password here (8+ characters).</div>
      <div className="row"><input type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="new password" style={{ width: 200 }} /><button className="btn small primary" disabled={pw.length < 8} onClick={() => auth.setPassword(pw).then(({ error }) => { if (error) say(error.message); else { setPw(""); say("Password saved"); } })}>Save password</button></div>
    </div>
    <div className="panel">
